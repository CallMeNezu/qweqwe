...
#include <iostream>
#include "windows.h"
void interface_buttons(int &a, int& b, int& c, int& d) {
	bool click = false;
    char plus_point_to_a_specific_stat;
    while (true) {
        if (GetAsyncKeyState('e')) //если кнопочка "е" нажата то открыть статистику
        {
            click = true;
        }
        else if (GetAsyncKeyState('r')) //если кнопочка "r" нажата то закрыть статистику 
        {
            click = false;
        }
        if (click == true) {
            while (plus_point_to_a_specific_stat != 'q') {
